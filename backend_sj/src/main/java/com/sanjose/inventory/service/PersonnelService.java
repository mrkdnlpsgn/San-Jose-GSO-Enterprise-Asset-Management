...
public class PersonnelService {
    ...
    public void delete(Long id) {
        ...
        // assets.personnel_id is a NO ACTION FK (same style as assets.office_id) —
        // deleting while assets are still accountable to this person would otherwise
        // fail as a raw SQL constraint error.
        Integer assetCount = jdbcTemplate.queryForObject(
            "SELECT COUNT(*) FROM assets WHERE personnel_id = ?", Integer.class, id);
        if (assetCount != null && assetCount > 0) {
            throw new ResourceInUseException(
                "Cannot delete \"" + personnel.getFullName() + "\" — " + assetCount +
                " asset(s) are still accountable to them. Reassign those assets first.");
        }
